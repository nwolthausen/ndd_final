...
label(table1_ndd$ndd_ada.hh_income)<- "Household income"
  ...
  ndd_table1<- table1(~ndd_ada.age_months_b + ndd_ada.sex + ndd_ada.primary_matedu + 
         ndd_ada.hh_income + Home + Household_persons + Chicken + cellphone + 
         elec+ floor + fridge + roof + toilet + tv + WI_quintile|ndd_ada.diarrhea_any_type_b, 
         overall=F, rowlabelhead = "Variable", footnote='SD = standard deviation',
         caption= "Table 1: Socioeconomic Determinants of Diarrheal Disease in Haitian Children",
         data=table1_ndd)
...
```
```{r}
#Multivariate model with diarrhea as the dependent variable and Wealth index quintiles, maternal education, household income, age and sex as the independent variables

MV_wi_quintlogit<- glm(ndd_ada.diarrhea_any_type_b ~ WI_quintile +
                      ndd_ada.primary_matedu + ndd_ada.age_months_b + ndd_ada.hh_income + ndd_ada.sex,
                       data = complete_ndd,family = "binomial")
summary(MV_wi_quintlogit)

odds.n.ends(MV_wi_quintlogit)


#Model is significant but WI is not a significant predictor of diarrhea

#When controlling for all other covariates, children whose mothers have a secondary education have 63% decrease in risk for diarrhea compared to those whose mothers have a primary education or less

#When controlling for all other covariates, children whose household income is >2,000 are 5 times the odds of children whose household income is 500-1000 of having diarrhea

#when controll for all other covariates, females have a 49% decreased risk of diarrhea compared to males

```
```{r}

#model with interaction terms
MV_wi_INTlogit_age_WI<- glm(ndd_ada.diarrhea_any_type_b ~ WI_quintile*ndd_ada.age_months_b + ndd_ada.primary_matedu + ndd_ada.sex + ndd_ada.hh_income, data = complete_ndd,
                    family = "binomial")
summary(MV_wi_INTlogit_age_WI)

odds.n.ends(MV_wi_INTlogit_age_WI) #Age

MV_wi_INTlogit_sex_WI<- glm(ndd_ada.diarrhea_any_type_b ~ ndd_ada.age_months_b + ndd_ada.primary_matedu + ndd_ada.sex*WI_quintile + ndd_ada.hh_income, data = complete_ndd,
                    family = "binomial")
summary(MV_wi_INTlogit_sex_WI)

odds.n.ends(MV_wi_INTlogit_sex_WI) #Sex

MV_wi_INTlogit_edu_WI<- glm(ndd_ada.diarrhea_any_type_b ~ WI_quintile*ndd_ada.primary_matedu + ndd_ada.age_months_b + ndd_ada.sex + ndd_ada.hh_income, data = complete_ndd,
                    family = "binomial")
summary(MV_wi_INTlogit_age_WI)
...
#influential points
plot(MV_wi_quintlogit, which=4, id.n=5, col="red") #obs 4 will be removed
...
#no significant change, observation will be left in the model
```
