...
table1_ndd %>%
  ndd_table1<- table1(~ndd_ada.age_months_b + ndd_ada.sex + ndd_ada.primary_matedu + 
         ...
         caption= "Table 1: Socioeconomic Determinants of Diarrheal Disease in Haitian Children", data=table1_ndd)

rownames(ndd_table1) = c("Age (months)", "Sex", "Maternal Education", 
                         "Household Income", "Rent/Own Home", "Chicken(s)", 
                         "Cellphone", "Access to electricity", "Floor type", 
                         "Refrigerator", "Roof type", "Toilet type", "Television", "Wealth index quintile*")
ndd_table1
...
MV_wi_quintlogit<- glm(ndd_ada.diarrhea_any_type_b ~ WI_quintile  + ndd_ada.hh_income +
                      ndd_ada.primary_matedu + ndd_ada.age_months_b + ndd_ada.sex,
...
MV_wi_INTlogit<- glm(ndd_ada.diarrhea_any_type_b ~ WI_quintile + ndd_ada.primary_matedu + ndd_ada.sex*ndd_ada.age_months_b, data = complete_ndd,
                    family = "binomial")
summary(MV_wi_INTlogit)

ORwi_INT<-exp(cbind(OR = coef(MV_wi_INTlogit), confint(MV_wi_INTlogit))) 
ORwi_MV 

odds.n.ends(MV_wi_INTlogit)
